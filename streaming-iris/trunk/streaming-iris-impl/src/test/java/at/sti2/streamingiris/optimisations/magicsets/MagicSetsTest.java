...
import at.sti2.streamingiris.api.IProgramOptimisation.Result;
...
public class MagicSetsTest extends TestCase {
	...
	private static IRule seedRule(final ILiteral l) {
		...
		return BASIC.createRule(Arrays.asList(l),
	...
	public void testMagic0() throws Exception {
		...
		ref.add(seedRule(createMagicLiteral("sg", BF,
				new ITerm[] { TERM.createString("john") })));
		...
		List<ILiteral> body = Arrays.asList(
				createMagicLiteral("sg", BF, new ITerm[] { X }),
	...
	public void testMagic1() throws Exception {
		...
		ref.add(seedRule(createMagicLiteral(
				"a",
				bbf,
				new ITerm[] { TERM.createString("john"),
	...
	public void testBoundConstant() throws Exception {
		...
		// magic_a_bf('john') :- .
		ref.add(seedRule(createMagicLiteral("a", bf,
				new ITerm[] { TERM.createString("john") })));
	...
	public void testConjunctiveQuery0() throws Exception {
		...
		final ILiteral p2 = BASIC.createLiteral(
				true,
				BASIC.createAtom(BASIC.createPredicate("p", 2),
						BASIC.createTuple(X, TERM.createString("a"))));
		final ILiteral r3 = BASIC.createLiteral(
				true,
				BASIC.createAtom(BASIC.createPredicate("r", 3),
						BASIC.createTuple(TERM.createString("b"), X, Y)));

		final List<IRule> ref = new ArrayList<IRule>();
		// magic_r_bbf(b, X) :- p(X, a)
		ref.add(BASIC.createRule(
				Arrays.asList(createMagicLiteral("r", bbf,
						new ITerm[] { TERM.createString("b"), X })),
				Arrays.asList(p2)));
		// magic_s_bb(e, Y) :- p(X, a), r(b, X, Y)
		ref.add(BASIC.createRule(
				Arrays.asList(createMagicLiteral("s", bb,
		...
		ref.add(BASIC.createRule(
				Arrays.asList(createLiteral("r", "X", "Y", "Z")),
		...
		// p_fb('a') :- .
		ref.add(seedRule(createMagicLiteral("p", fb,
				new ITerm[] { TERM.createString("a") })));
	...
	public void testConjunctiveQuery1() throws Exception {
		...
		final ILiteral r3 = BASIC.createLiteral(
				true,
				BASIC.createAtom(BASIC.createPredicate("r", 3),
						BASIC.createTuple(TERM.createString("b"), X, Z)));
	...
	public void testConjunctiveQuery2() throws Exception {
		...
		final ILiteral p2 = BASIC.createLiteral(
				...
				BASIC.createAtom(
						...
						BASIC.createTuple(TERM.createString("b"),
								TERM.createString("a"))));
		final ILiteral r3 = BASIC.createLiteral(
				true,
				BASIC.createAtom(BASIC.createPredicate("r", 3),
		...
		// magic_r_bff(b) :- p(b, a)
		ref.add(BASIC.createRule(
				Arrays.asList(createMagicLiteral("r", bff,
		...
		ref.add(BASIC.createRule(Arrays.asList(createLiteral("s", "X", "Y")),
				Arrays.asList(createMagicLiteral("s", bb, XY), c2)));
		// p^ff() :- TRUE
		ref.add(seedRule(createMagicLiteral("p", bb,
	...
	public void testComplicatedConjunctiveQuery0() throws Exception {
		...
		ref.add(BASIC.createRule(
				Arrays.asList(createMagicLiteral("r", fbb, YX)),
				Arrays.asList(c2)));
		// magic_s_bb(e, Z) :- p(X, Y), r(b, X, Z)
		ref.add(BASIC.createRule(
				Arrays.asList(createMagicLiteral("s", bb,
	...
	public void testRepeatedLiteralQuery() throws Exception {
		...
		final ILiteral p1 = BASIC.createLiteral(
				true,
	...
	public void testNegativeConjunctiveQuery() throws Exception {
		...
		rules.add(BASIC.createRule(
				Arrays.asList(createMagicLiteral("q", BB, X3)),
				Arrays.asList(createLiteral("p", "X", "Y"))));
		// p(?X, ?Y) :- a(?X, ?Y).
		...
		rules.add(BASIC.createRule(
				Arrays.asList(createLiteral("q", "X", "Y")),
				Arrays.asList(createMagicLiteral("q", BB, XY),
						createLiteral("b", "X", "Y"))));

		// ?- p(?X, ?Y), !q(?X, 3).
		final IQuery query = BASIC.createQuery(Arrays.asList(
				createLiteral("p", "X", "Y"),
				BASIC.createLiteral(false, BASIC.createPredicate("q", 2),
						BASIC.createTuple(X3))));
	...
	public void testUnsaveLabeledRuleCreation() throws Exception {
		...
		rules.add(BASIC.createRule(
				Arrays.asList(createLiteral("c", "X", "Y")),
				Arrays.asList(createMagicLiteral("c", BB, XY),
						createLiteral("e", "X", "Y"))));
		// magic_a_bf(2) :- .
		rules.add(seedRule(createMagicLiteral("a", BF,
				new ITerm[] { CONCRETE.createInteger(2) })));
		...
		final IQuery query = BASIC.createQuery(Arrays.asList(BASIC
				.createLiteral(true, BASIC.createPredicate("a", 2),
	 ...
	 * 
	 * @see <a href=
	 *      "https://sourceforge.net/tracker/index.php?func=detail&aid=1919554&group_id=167309&atid=842434"
	...
	public void testKeepBuiltins() throws Exception {
		...
		rules.add(BASIC.createRule(Arrays.asList(createLiteral("a", "X", "Y")),
				Arrays.asList(
						...
						BASIC.createLiteral(
								true,
								BUILTIN.createAddBuiltin(Z,
										CONCRETE.createInteger(1), Y)))));
		// magic_a_bf('john') :- .
	...
	public void testConjunctiveBuiltinAdorning() throws Exception {
		...
		// ?- a('john'), EQUAL(?X, 'john').
		final IQuery query = BASIC.createQuery(Arrays.asList(
				BASIC.createLiteral(true, BASIC.createPredicate("a", 1),
	...
	public static Test suite() {
		return new TestSuite(MagicSetsTest.class,
				MagicSetsTest.class.getSimpleName());
	}
}
