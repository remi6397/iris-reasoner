...
public class SLDNFEvaluator implements ITopDownEvaluator {
	 ...
	 */
	private IRelation findAndSubstitute(IQuery query, Integer recursionDepth, boolean inNegationAsFailureFlip) throws EvaluationException, MaximumRecursionDepthReachedException {
		...
		ILiteral queryLiteral = standardSelector.select(query.getLiterals());
		
		if (queryLiteral == null)
			throw new EvaluationException("The selected literal must not be null.");
		
		if (DEBUG)
			System.out.println(debugPrefix + "Selected: " + queryLiteral);
		
		// The results are stored in this relation
		IRelation relationReturned = null;
		
		// Process selected literal
		// Every possibility (every child/subtree of this node) is stored in a list, which is iterated later.
		if (queryLiteral.isPositive()) { // Positive Query Literal - search for a success node
			
			List<QueryWithSubstitution> newQueryList = new LinkedList<QueryWithSubstitution>(); // List of new queries (incl. substitutions) to process, generated by facts and rules
					
			
			IAtom queryLiteralAtom = queryLiteral.getAtom();
			
			if (queryLiteralAtom  instanceof IBuiltinAtom) { // BuiltIn
				newQueryList.addAll( processBuiltin(query, queryLiteral, queryLiteralAtom) );
			} else { // Not BuiltIn				
				newQueryList.addAll( processQueryAgainstFacts(query, queryLiteral) );
				newQueryList.addAll( processQueryAgainstRules(query, queryLiteral) );
			}
			
			for (QueryWithSubstitution qws : newQueryList) { // process new queries
				...
				if (newQuery.getLiterals().isEmpty()) {					
					relationReturned = (relationReturned == null) ? srf.createRelation() : relationReturned; 
			...
			ILiteral queryLiteralNAF = Factory.BASIC.createLiteral(true, queryLiteral.getAtom());
			...
			if (DEBUG)
				System.out.println(debugPrefix + "NAF Subtree: " + query + " = " + relationFromNAFSubtree);
			
			if (relationFromNAFSubtree == null) {
				...
				literalsWithoutNegatedLiteral.remove( queryLiteral );
	...
	private IRelation getFullSubgoalRelation(IQuery query, QueryWithSubstitution qws,
		...
		if (relationFromSubtree.size() == 0) {
			...
			relation.add(tuple);
				 ...
				 // (means that this is a parent of a successor node)
			...
			for (int i=0; i < relationFromSubtree.size(); i++) {
				...
				for (IVariable var : queryVariableList) {
					...
					if ( termFromVariableMapping == null && newQueryVariableList.contains(var) ) {
						...
						if (branchTuple.size() != newQueryVariableList.size())
							continue;
						
						ITerm termToAdd = branchTuple.get( newQueryVariableList.indexOf(var) );
	 ...
	 */
	private String getDebugPrefix(Integer recursionDepth,
