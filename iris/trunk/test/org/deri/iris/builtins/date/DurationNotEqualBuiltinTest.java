...
public class DurationNotEqualBuiltinTest extends AbstractDateBuiltinTest {
	...
	public void testBuiltin() throws EvaluationException {
		...
		ITerm duration3 = Factory.CONCRETE.createDuration(true, 1979, 10, 28,
				12, 56, 23);
		ITerm duration4 = Factory.CONCRETE.createDuration(false, 1991, 1, 8, 12,
				56, 23);

		DurationNotEqualBuiltin builtin = new DurationNotEqualBuiltin(
				duration1, duration1);
		...
		builtin = new DurationNotEqualBuiltin(duration1, duration2);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// duration1 = duration2
		assertEquals(null, actual);

		builtin = new DurationNotEqualBuiltin(duration1, duration3);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// duration1 > duration3
		assertEquals(EMPTY_TUPLE, actual);

		builtin = new DurationNotEqualBuiltin(duration3, duration4);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// duration3 < duration4
		assertEquals(EMPTY_TUPLE, actual);
