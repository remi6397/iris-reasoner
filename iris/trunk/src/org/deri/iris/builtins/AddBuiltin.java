 ...
 * </p>
 * <p>
 * $Id: AddBuiltin.java,v 1.14 2007-10-10 14:47:06 bazbishop237 Exp $
 * </p>
 * 
...
public class AddBuiltin extends AbstractBuiltin {
	...
	public ITuple evaluate(final ITuple c) {
		...
		if (vars.length == 0) { // checking whether the result is correct
			ITerm result = BuiltinHelper.add(complete[0], complete[1]);
		...
		else if( vars.length == 1 )
		{
			...
			// calculate the missing position
			switch( vars[0] )
			{
				case 0:
					result = BuiltinHelper.subtract(complete[2], complete[1]);
					break;
					
				case 1:
					result = BuiltinHelper.subtract(complete[2], complete[0]);
					break;
					
				case 2:
					result = BuiltinHelper.add(complete[0], complete[1]);
					break;
					
				default:
					throw new IllegalArgumentException("The variable must be at position " + 
