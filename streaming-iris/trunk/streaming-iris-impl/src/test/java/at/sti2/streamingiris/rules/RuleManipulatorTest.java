...
public class RuleManipulatorTest extends TestCase {
	ILiteral createLiteral(boolean positive, String predicateName,
		...
		return BASIC.createLiteral(positive, BASIC.createAtom(
	...
	public void testReplaceTermInAtom() {
		...
		// f(?X)
		IAtom atom = BASIC.createAtom(BASIC.createPredicate("f", 1),
				BASIC.createTuple(X));
	...
	public void testReplaceTermInAtomWithConstructedTerm() {
		...
		IAtom atom = BASIC.createAtom(BASIC.createPredicate("q", 2),
				BASIC.createTuple(constructedTerm, X));
	...
	public void testReplaceTermInAtomWithDoubleConstructedTerm() {
		...
		// q( f(g(?X)) )
		IConstructedTerm constructedTerm = TERM.createConstruct("f",
				TERM.createConstruct("g", X));
		IAtom atom = BASIC.createAtom(BASIC.createPredicate("q", 1),
				BASIC.createTuple(constructedTerm));

		// Replace the f(g(?X)) in q( f(g(?X)) ) ...
		IConstructedTerm fgX = TERM.createConstruct("f",
				TERM.createConstruct("g", X));
		IConstructedTerm fgX1 = TERM.createConstruct("f",
				TERM.createConstruct("g", X1));
	...
	public void testReplaceTermInAtomWithDoubleConstructedTermNestedReplace() {
		...
		IConstructedTerm constructedTerm = TERM.createConstruct("f",
				TERM.createConstruct("g", X));
		IAtom atom = BASIC.createAtom(BASIC.createPredicate("q", 2), BASIC
				.createTuple(constructedTerm, TERM.createConstruct("g", X)));
	...
	public void testAll() {
		...
		IRule rule4 = rm.addEquality(rule3, TERM.createVariable("Y"),
