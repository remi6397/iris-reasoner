...
public class SimpleRdbRelation extends AbstractRdbRelation {
	...
	public SimpleRdbRelation(Connection connection, String tableName, int arity)
			throws SQLException {
		super(connection);
	...
	public boolean addAll(IRdbRelation otherRelation) throws SQLException {
		...
		// Avoid an unique index violation by filtering out those tuples from
		// the source relation that are already in the target relation.
		...
		String sql = String.format(sqlFormat, getTableName(),
				attributes.toString(), selectClause);

		Connection connection = getConnection();
		CallableStatement call = null;

		try {
			logger.debug("Adding tuples from " + otherRelation.getTableName()
					+ " to " + getTableName());
			...
			logger.error(
					"Failed to add tuples from " + otherRelation.getTableName()
							+ " to " + getTableName(), e);
			throw e;
		}
	}
	...
	private String createSelectJoinClause(IRdbRelation otherRelation) {
		...
		for (int i = 0; i < getArity(); i++) {
			String leftAttribute = LEFT_TABLE_ALIAS + "."
					+ rightAttributeList.get(i);
			String rightAttribute = getTableName() + "."
					+ rightAttributeList.get(i);
		...
		if (joinParts.size() > 0) {
			...
			String join = String.format(joinFormat, getTableName(),
		...
		String sql = String.format(sqlFormat, leftAttributes.toString(),
				otherRelation.getTableName() + " AS " + LEFT_TABLE_ALIAS, join);
	...
	public ITuple get(int index) {
		...
		try {
			...
			// In H2 the first row has the row number 1.
	...
	private void createGetTupleStatement() throws SQLException {
		if (getTupleStatement == null) {
			String sqlFormat = "SELECT * FROM "
					+ "(SELECT *, ROWNUM AS num FROM %s) WHERE num = ?";
			String sql = String.format(sqlFormat, getTableName());
	...
	@Override
	public void drop() {
		// There is nothing to drop, so we only close the relation.
		close();
	}
