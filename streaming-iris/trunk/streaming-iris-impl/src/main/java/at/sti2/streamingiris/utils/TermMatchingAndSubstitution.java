...
/**
 * A collection of utility methods for term/tuple matching and variable
 * substitution.
 */
public class TermMatchingAndSubstitution {
	/**
	 * Extract the variables in the same order that they are discovered during
	 * term matching.
	 * 
	 * @param subGoalTuple
	 *            The tuple as it appears in the sub-goal
	 * @param unique
	 *            true, if only unique variables are required
	 ...
	 * 
	 * @param constructed
	 *            The constructed term to find the variables for.
	 * @param variables
	 *            The map to put the variables as they are found.
	 */
	 ...
	 * 
	 * @param term
	 *            The term to extract the variables from.
	 * @param unique
	 *            If false, include each variable every time it appears.
	...
	/**
	 * Match a tuple to view criteria. If a match occurs, return a tuple with
	 * values for each distinct variable in the view criteria.
	 * 
	 * @param viewCriteria
	 *            The tuple from a sub-goal instance.
	 * @param relation
	 *            The tuple from an EDB relation.
	 * @return The tuple of values for the view's variables or null if a match
	 *         did not occur.
	 */
	...
	public static ITuple matchTuple(ITuple viewCriteria, ITuple relation,
		...
		for (int i = 0; i < viewCriteria.size(); ++i) {
			...
			if (!matchTermOfTuple(bodyTerm, relationTerm, equivalentTerms,
					variableMap, terms))
				return null;
		}
	 ...
	 * 
	 * @param viewTerm
	 *            The term from the rule's sub-goal tuple.
	 * @param relationTerm
	 *            The term from the relation's tuple.
	 * @param equivalentTerms
	 *            The equivalent terms.
	 * @param variableMap
	 *            The current map of variable-constant bindings.
	 * @param terms
	 *            The bound variable values in the order in which they are
	 *            found.
	 * @return true, if these terms match and are consistent with previous
	 *         macthed term pairs.
	 */
	private static boolean matchTermOfTuple(ITerm viewTerm, ITerm relationTerm,
			IEquivalentTerms equivalentTerms,
			Map<IVariable, ITerm> variableMap, List<ITerm> terms) {
		if (viewTerm instanceof IVariable) {
			...
			if (mappedGroundTerm == null) {
				...
				return true;
					...
					|| equivalentTerms.areEquivalent(mappedGroundTerm,
							relationTerm)) {
			...
			if (relationTerm instanceof IConstructedTerm) {
				...
				if (!bodyConstructedTerm.getFunctionSymbol().equals(
						relationConstructedTerm.getFunctionSymbol()))
					return false;

				List<ITerm> bodyTerms = bodyConstructedTerm.getParameters();
				List<ITerm> relationTerms = relationConstructedTerm
						.getParameters();
				...
				for (int i = 0; i < bodyTerms.size(); ++i) {
					if (!matchTermOfTuple(bodyTerms.get(i),
							relationTerms.get(i), equivalentTerms, variableMap,
							terms))
						return false;
				}
				return true;
			...
			return viewTerm.equals(relationTerm)
					|| equivalentTerms.areEquivalent(viewTerm, relationTerm);
		}
	}

	/**
	 * Indicates if bodyTerm (containing >= 0 variables) matches groundTerm and
	 * is also consistent with previous matched terms, i.e. variables map to the
	 * same ground terms.
	 * 
	 * @param bodyTerm
	 *            The term from the sub-goal predicate
	 * @param relationTerm
	 *            The term from the relation tuple
	 * @param variableMap
	 *            The map of variable to ground term map
	 * @return true if the match succeeds, false otherwise
	 */
	public static boolean match(ITerm bodyTerm, ITerm relationTerm,
		...
		if (bodyTerm instanceof IVariable) {
			...
			if (relationTerm instanceof IConstructedTerm) {
				...
				for (int i = 0; i < bodyTerms.size(); ++i) {
					if (!match(bodyTerms.get(i), relationTerms.get(i),
							variableMap))
						return false;
				}
				return true;
	 ...
	 * check if it is necessary to add a atom to the memo table or if an
	 * equivalent atom is already in the memo table. Unification is not
	 * sufficient for this task, since e.g. p(?X) and p(1) are unifiable, but
	 * are likely to have different answers.
	 * 
	 * @param tup1
	 *            tuple one
	 * @param tup2
	 ...
	 *            tuple two
	 * @param variableMap
	 *            Map where variable bindings are saved
	 * 
	 * @return true if it is unifiable, false if not
	 * 
	 * @author gigi
	 */
	public static boolean unify(ITuple tup1, ITuple tup2,
			Map<IVariable, ITerm> variableMap) {
	 ...
	 *            Do the unification in both directions. Actually an additional
	 *            flag used by the subsums() method.
	 */
	public static boolean unify(ITuple tup1, ITuple tup2,
			Map<IVariable, ITerm> variableMap, boolean bothDirections) {
	 ...
	 */
	public static boolean unify(ITerm t1, ITerm t2,
			Map<IVariable, ITerm> variableMap) {
	 ...
	 */
	public static boolean unify(ITerm t1, ITerm t2,
			Map<IVariable, ITerm> variableMap, IEquivalentTerms equivalentTerms) {
	...
	public static boolean unify(ITerm t1, ITerm t2,
			Map<IVariable, ITerm> variableMap, boolean bothDirections) {
		return unify(t1, t2, variableMap, bothDirections,
				new IgnoreTermEquivalence());
	}
		...
		if (t1 instanceof IConstructedTerm && t2 instanceof IConstructedTerm) {
			...
			for (int i = 0; i < c1terms.size(); ++i) {
				...
				boolean termUnifiy = unify(c1term, c2term, variableMap,
						bothDirections, equivalentTerms);
				if (!termUnifiy)
					return false;
			}
			return true;
	...
	private static boolean unifyCheckBinding(IVariable variable, ITerm term,
			Map<IVariable, ITerm> variableMap, IEquivalentTerms equivalentTerms) {
	 ...
	 * 
	 * @param tuple
	 *            The tuple containing variables to ground.
	 * @param variableMap
	...
	public static ITuple substituteVariablesInToTuple(ITuple tuple,
		...
		for (ITerm headTerm : tuple) {
			ITerm substitutedTerm = substituteVariablesInToTerm(headTerm,
					variableMap);
	...
	public static ITerm substituteVariablesInToTerm(ITerm term,
		...
		for (ITerm childTerm : constructedTerm.getParameters())
			substitutedChildTerms.add(substituteVariablesInToTerm(childTerm,
					variableMap));

		return Factory.TERM.createConstruct(
				constructedTerm.getFunctionSymbol(), substitutedChildTerms);
	}
	...
	/**
	 * Substitute variable values in to a tuple to ground it using a list of
	 * terms with indices instead of a variable-term map.
	 * 
	 * @param tuple
	 *            The tuple containing variables to ground.
	 ...
	 * @param indices
	 *            The indices in to variableValues for each occurrence of a
	 *            variable in the tuple IN THE ORDER IN WHICH THEY ARE FOUND. An
	 *            index value that is less than zero indicates that there is no
	 *            binding for this variable.
	 * @return The grounded tuple.
	 */
	public static ITuple substituteVariablesInToTuple(ITuple tuple,
			List<ITerm> variableValues, int[] indices) {
		List<ITerm> substitutedTerms = new ArrayList<ITerm>();
		MutableInteger bindingIndex = new MutableInteger();

		for (ITerm headTerm : tuple) {
			ITerm substitutedTerm = substituteVariablesInToTupleTerm(headTerm,
	 ...
	 * 
	 * @param term
	 *            The term to substitute in to.
	 * @param variableValues
	 *            The variable value list.
	 * @param indices
	 *            The indices in to variableValues for each occurence of a
	 *            variable.
	 * @param bindingIndex
	 *            Indicates the next index to use.
	 * @return The grounded term.
	 */
	private static ITerm substituteVariablesInToTupleTerm(ITerm term,
			List<ITerm> variableValues, int[] indices,
			MutableInteger bindingIndex) {
		// Recursion terminator. Probably inefficient as tree is traversed
		// anyway.
		if (term.isGround())
			return term;
		...
		for (ITerm childTerm : constructedTerm.getParameters())
			substitutedChildTerms.add(substituteVariablesInToTupleTerm(
					childTerm, variableValues, indices, bindingIndex));
	...
	/**
	 * Substitute variable values in to a term to ground it using variable
	 * bindings as a list of terms with indices instead of a variable-term map.
	 * 
	 * @param term
	 *            The term to ground.
	 * @param variableValues
	 ...
	 * @param indices
	 *            The indices in to variableValues for each occurrence of a
	 *            variable in the term IN THE ORDER IN WHICH THEY ARE FOUND. An
	 *            index value that is less than zero indicates that there is no
	 *            binding for this variable.
	 * @return The grounded term.
	 */
	public static ITerm substituteVariablesInToTerm(ITerm term,
			List<ITerm> variableValues, int[] indices) {
		return substituteVariablesInToTupleTerm(term, variableValues, indices,
				new MutableInteger());
	}
}
