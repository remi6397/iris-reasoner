...
/**
 * The global stratification algorithm. This algorithm does not
 * modify/create/delete any rules.
 * 
...
public class GlobalStratifier implements IRuleStratifier {
		...
		if (highest <= ruleCount) {
			...
			for (Map.Entry<IPredicate, Integer> entry : mStrata.entrySet()) {
				...
				for (IRule rule : rules) {
					...
					if (rule.getHead().get(0).getAtom().getPredicate()
							.equals(predicate)) {
	...
	private void setStratum(final IPredicate predicate, final int stratum) {
		assert predicate != null;
		assert stratum >= 0 : "The stratum must not be negative, but was: "
				+ stratum;
	...
	/**
	 * Checks if a rule with head equality is stratified. A program is
	 * stratified if and only if rules with head equality are in the bottom
	 * stratum and they don't have negated predicates.
	 * 
	 * @param rule
	 *            The rule to check if stratified.
	 * @param stratum
	 *            The stratum of the rule.
	 * @return <code>true</code> if the rule is stratified, <code>false</code>
	 *         otherwise.
	 */
