...
public class MultiplyBuiltin extends AbstractBuiltin {
	...
	public List<ITuple> evaluate(final Collection<ITuple> c) {
		...
		for(final ITuple t : c) {
			...
			if(vars[0] == 0) {
				complete[vars[0]] = BuiltinHelper.divide(complete[2], complete[1]);
			} else if (vars[0] == 1) {
				complete[vars[0]] = BuiltinHelper.divide(complete[2], complete[0]);
			} else if (vars[0] == 2) {
				complete[vars[0]] = BuiltinHelper.multiply(complete[0], complete[1]);
			} else {
