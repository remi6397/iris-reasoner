...
public class DayTimeDurationGreaterBuiltinTest extends AbstractDateBuiltinTest {
	...
	public void testBuiltin() throws EvaluationException {
		...
		DayTimeDurationGreaterBuiltin builtin = new DayTimeDurationGreaterBuiltin(date1, date1);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date1 = date1) -> null
		assertEquals(null, actual);
		
		builtin = new DayTimeDurationGreaterBuiltin(date1, date2);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date1 = date2) -> null
		assertEquals(null, actual);

		builtin = new DayTimeDurationGreaterBuiltin(date3, date4);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date3 > date4) -> iTuple()
		assertEquals(EMPTY_TUPLE, actual);

		builtin = new DayTimeDurationGreaterBuiltin(date4, date3);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date4 < date3) -> null
		assertEquals(null, actual);
		
		builtin = new DayTimeDurationGreaterBuiltin(date4, date5);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
