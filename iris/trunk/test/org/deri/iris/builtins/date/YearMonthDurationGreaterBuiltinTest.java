...
public class YearMonthDurationGreaterBuiltinTest extends
	...
	public void testBuiltin() throws EvaluationException {
		...
		builtin = new YearMonthDurationGreaterBuiltin(date1, date2);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date1 = date2) -> null
		assertEquals(null, actual);

		builtin = new YearMonthDurationGreaterBuiltin(date3, date4);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date3 < date4) -> null
		assertEquals(null, actual);

		builtin = new YearMonthDurationGreaterBuiltin(date4, date3);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date4 > date3) -> EMPTY_TUPLE
		assertEquals(EMPTY_TUPLE, actual);

		builtin = new YearMonthDurationGreaterBuiltin(date4, date5);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date4 > date5) -> null
		assertEquals(EMPTY_TUPLE, actual);
