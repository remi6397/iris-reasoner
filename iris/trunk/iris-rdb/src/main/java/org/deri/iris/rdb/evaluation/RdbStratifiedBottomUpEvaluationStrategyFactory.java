...
/**
 * Factory that creates {@link RdbStratifiedBottomUpEvaluationStrategy}
 * objects.
 */
public class RdbStratifiedBottomUpEvaluationStrategyFactory implements
	...
	public RdbStratifiedBottomUpEvaluationStrategyFactory(
			Connection connection, IRuleEvaluatorFactory ruleEvaluatorFactory) {
	...
	public IEvaluationStrategy createEvaluator(IFacts facts, List<IRule> rules,
		...
		return new RdbStratifiedBottomUpEvaluationStrategy(connection, facts,
				rules, ruleEvaluatorFactory, configuration);
	}
