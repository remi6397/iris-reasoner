...
public class DateLessEqualBuiltinTest extends AbstractDateBuiltinTest {
	...
	public void testBuiltin() throws EvaluationException {
		...
		DateLessEqualBuiltin builtin = new DateLessEqualBuiltin(date1, date2);
		ITuple args = Factory.BASIC.createTuple(X);
		ITuple actual = builtin.evaluate(args);
		// (date1 = date2) -> null
		assertEquals(EMPTY_TUPLE, actual);

		builtin = new DateLessEqualBuiltin(date3, date4);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date3 > date4) -> iTuple()
		assertEquals(null, actual);

		builtin = new DateLessEqualBuiltin(date4, date3);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date4 < date3) -> null
		assertEquals(EMPTY_TUPLE, actual);
