...
public class RuleAnalyser {
	 ...
	 * @param rule
	 *            the rule to check
	 * @return <code>true</code> if the rule body contains head atoms, otherwise
	 *         <code>false</code>
	...
	public static boolean hasHeadLiteralInBody(final IRule rule) {
		...
		if (rule.getHead().size() > 1) {
			throw new IllegalArgumentException(
					"The rule to check must contain only one head");
		}
	...
	/**
	 * <p>
	 * Checks whether it is possible to assign any values to variables by static
	 * analysis of the rule.
	 * </p>
	 * <p>
	 * <b>The implementation of this method is not finished. At the moment it is
	 * not able to handle constructed terms.</b>
	 * </p>
	 * 
	 * @param rule
	 *            the rule to check
	 ...
	 *             if something went wrong while evaluating one of the equality
	 *             built-ins.
	 */
	public static boolean hasSatisfiableVariableAssignment(final IRule rule)
			throws EvaluationException {
		if (rule == null) {
			throw new IllegalArgumentException("The rule must not be null");
		}
	...
	private static boolean unifies(ITuple tuple) {
		...
		return TermMatchingAndSubstitution.unify(tuple.get(0), tuple.get(1),
				variableMap);
	}
