...
public class YearMonthDurationLessEqualBuiltinTest extends
	...
	public void testBuiltin() throws EvaluationException {
		...
		YearMonthDurationLessEqualBuiltin builtin = new YearMonthDurationLessEqualBuiltin(
				date1, date1);
		...
		builtin = new YearMonthDurationLessEqualBuiltin(date1, date2);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date1 = date2) -> EMPTY_TUPLE
		assertEquals(EMPTY_TUPLE, actual);

		builtin = new YearMonthDurationLessEqualBuiltin(date3, date4);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date3 < date4) -> EMPTY_TUPLE
		assertEquals(EMPTY_TUPLE, actual);

		builtin = new YearMonthDurationLessEqualBuiltin(date4, date3);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date4 > date3) -> null
		assertEquals(null, actual);

		builtin = new YearMonthDurationLessEqualBuiltin(date4, date5);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date4 > date5) -> null
		assertEquals(null, actual);
