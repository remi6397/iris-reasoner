...
public class QSQEvaluator implements IEvaluator{
	...
	private Map<IQuery, IRelation> results = null;
	...
	public QSQEvaluator(IEDB edb){
		...
		this.edb = edb;
		this.results = new HashMap<IQuery, IRelation>();
	}
	...
	public boolean evaluate() throws DataModelException {
		...
		while(qi.hasNext()){
			 ...
			 * the global variables (inputs and outputs). However for each 
			 * loop we need to consider only new tuples in inputs 
			 * global variables (ones that have not been considered yet).  
			 */
			while(contEval){
				...
				while(ri.hasNext()){
					r = ri.next();
					ap_inner = isAdorned(r.getAdornedRule().getHeadLiteral(0).getPredicate());
	 ...
	 * Arity of the output relations is equal to the arity of head literal
	 * of the entire realtions.
	 */
		...
		while(i.hasNext()){
			...
			if(ap != null){
				...
				if(! this.outputs.containsKey(ap)){
					this.outputs.put(ap, new Relation(
							r.getAdornedRule().getHeadLiteral(0).getAtom().getTuple().getArity()));
				}
	...
	/**
	 * It takes a subquery (R^a, t), which is a pair of an adorned rule
	 * and a set of tuples t that have not been considered with this rule yet,
	 * and evaluate it.
	 *  
	 * @param r adorned rule to be evaluated.
	 */
			...
			for(ILiteral lit : rule.getAdornedRule().getBodyLiterals()){
				...
				if(ap == null){
					this.joinOperator = RELATION_OPERATION.createJoinOperator(
							rule.getSup_0(), this.edb.getFacts(lit.getPredicate()), 
							this.getJoinIndexes(rule.getSup_0().getVariables(), lit), 
							JoinCondition.EQUALS,
							this.getProjectIndexes0(
									rule.getSup_0().getVariables(), lit, rule.getSup_1(rule.getSup_0()).getVariables()));
				...
				// apply: 3(b)(i) 
					...
					this.projectionOperator = 
						RELATION_OPERATION.createProjectionOperator(
								rule.getSup_0(), this.getProjectIndexes1(
										rule.getSup_0().getVariables(), vars));
					...
					if(rel.size() > 0){
						...
						if(setInput(ap, rel)){
							...
							while(i.hasNext()){
								r_inner = i.next();
								ap_inner = isAdorned(
										r_inner.getAdornedRule().getHeadLiteral(0).getPredicate());
						...
						this.joinOperator = RELATION_OPERATION.createJoinOperator(
								rule.getSup_0(), this.outputs.get(ap), 
								this.getJoinIndexes(
										rule.getSup_0().getVariables(), lit), JoinCondition.EQUALS,
								this.getProjectIndexes0(
										rule.getSup_0().getVariables(), 
										lit, rule.getSup_1(rule.getSup_0()).getVariables()));
				...
				if(! rule.hasNext(rule.getSup_1(rule.getSup_0())) && rule.getSup_1(rule.getSup_0()).size()>0){
					ap = (AdornedPredicate)rule.getAdornedRule().getHeadLiteral(0).getPredicate();
					this.outputs.get(ap).addAll(rule.getSup_1(rule.getSup_0()));
					contEval = true;
				}
				if(rule.getSup_1(rule.getSup_0()).size() == 0) break;
	 ...
	 * @return  Adorned predicate of p (if p is an adorned predicate),
	 * 		    otherwise null.
	 */
	...
	/**
	 * @param vars list of variables of the supplementary relation
	 * @param l literal that needs to be joined with the 
	 * 			corresponding supplementary relation 
	...
	/**
	 * Case: B(i), page: 322.
	 * Defines an index array which the projection operation will be 
	 * based on.
	 * 
	 * @param vars0 Variables of a supplementary relation which has
	 * 		  		an index j-1 
	 * @param l		Literal that follows a supplementary relation which has
	 * 		  		an index j-1 
	 * @param vars1 Variables of a supplementary relation which has
	 * 		  		an index j
	 * @return		Projection indexes
	 */
	private int[] getProjectIndexes0(List<IVariable> vars0, 
			ILiteral l, List<IVariable> vars1){
	 ...
	 * @param vars0 Variables of a supplementary relation which has
	 * 		  		an index j-1 
	 * @param vars1 Variables of literal that follows a supplementary
	 * 				relation which has an index j 
	 * @return		Projection indexes
	 */
	private int[] getProjectIndexes1(
			List<IVariable> vars0, List<IVariable> vars1){
	...
	/**
	 * @param t tuple provided by a query
	 * @return  Returns initial tuple (created out of bound 
	 * 		    terms of the query) for beginning of a rule 
	 * 	        evaluation or null (If the query doesn’t 
	...
	/**
	 * Empties all input global relations. This operation is needed in 
	 * order to terminate the overall evaluation process. Namely the 
	 * evaluation is looping until no new tuples are added to any of 
	 * the global variables (inputs and outputs). After each loop we 
	 * empty input global variables (relation), so that we can check 
	 * whether new tuples will be added in the next loop. The 
	 * evaluation terminates when no new tuples are added to any of 
	 * global variables (relations).  
	 */
	private void emptyInputs(){
		...
		while(ri.hasNext()){
			r = ri.next();
			ap = (AdornedPredicate)r.getAdornedRule().getHeadLiteral(0).getPredicate();
	...
	/**
	 * Set sup_0 := T of the rule r.
	 * (T - set of tuples that have not been considered
	 * with the entire rule r yet, apply: 2...{(A)} 
	 * 
	 * @param r rule to be evaluated
	 */
	private IRelation setSup_0(QSQRule r){
		IRelation rel = null;
		if(r.getSupRels().get(0).size()>0){
			rel = new Relation(
					((ITuple)r.getSupRels().get(0).first()).getArity());
	...
	/**
	 * Sets input (sup_0 relation) for each rule r with 
	 * an adorned predicate ap. Only new tuples from 
	 * relatin rel are added.
	 * 
	 * @param ap  adorned predicate
	 * @param rel relation
	...
	private void setResult(){
		...
		while(qi.hasNext()){
			...
			while(pi.hasNext()){
				...
				if(Arrays.equals(ads, ap.getAdornment())){
					this.results.put(q, this.outputs.get(ap));
				}
			}
		}
	}
	...
	private void printResult(){
		...
		Iterator<IQuery> ri = null;
		IQuery q = null;
		
		ri = this.results.keySet().iterator();
		while(ri.hasNext()){
			q = ri.next();
			buffer.append("\n");
			buffer.append("The output for query: " + q.toString());
			i = this.results.get(q).iterator();
			while(i.hasNext()){
				buffer.append("\n");
	...
	/**
	 * @param t0 input tuple to be unified with t1
	 * @param t1 tuple of supplementary relation sr to be unified with t0
	 * @return   Ground tuple if t0 is unifyable with t1
	 */
	private ITuple unify(ITuple t0, ITuple t1){
		IUnification unifier = TUPLE_OPERATION.createUnificationOperator(t0, t1);
	...
	/**
	 * @param t Term to be grounded with unification result l
	 * @param l Unification result
	 * @return  Ground term
	 */
