...
class Joiner extends RuleElement
{
					...
					IRelationFactory relationFactory )
	{
		...
		// Now find the indices of variables that are not used in the natural join
	 ...
	 * @param remainderIndicesThisLiteral Indices of those terms not used in joining.
	 * @param outputVariables The output variable bindings for this literal.
	 */
	public Joiner(	IPredicate predicate,
					IRelation thisLiteralsRelation,
					ITuple viewCriteria,
					List<IVariable> viewVariables,
					boolean simple,
					int[] joinIndicesInput,
					int[] joinIndicesThisLiteral,
					int[] remainderIndicesInput,
					int[] remainderIndicesThisLiteral,
	...
	{
		// TODO
		// Optimisation possible: If either remainder indices are zero length then can just
		// return one or the other tuple. However, care must be taken during initialisation
		// to ensure that the output variable order is correct.
		ITerm[] terms = new ITerm[ mJoinIndicesInput.length + mRemainderIndicesInput.length + mRemainderIndicesThisLiteral.length ];
    ...
    public RuleElement getDeltaSubstitution( IFacts deltas )
    {
	    ...
	    return new Joiner( mPredicate, delta,
	    				mViewCriteria, mView.variables(), mView.isSimple(),
	    				mJoinIndicesInput,
						mJoinIndicesThisLiteral,
						mRemainderIndicesInput,
						mRemainderIndicesThisLiteral,
						mOutputVariables,
						mIndexFactory,
						mRelationFactory
	    				);
