...
import at.sti2.streamingiris.evaluation.topdown.MaximumRecursionDepthReachedException;
import at.sti2.streamingiris.evaluation.topdown.QueryWithSubstitution;
import at.sti2.streamingiris.evaluation.topdown.StandardLiteralSelector;
...
/**
 * Implementation of the SLDNF evaluator. Please keep in mind that SLDNF
 * evaluation is not capable of any tabling and can get trapped in an infinite
 * loop.
 * 
...
public class SLDNFEvaluator implements ITopDownEvaluator {
	...
	private IRelation findAndSubstitute(IQuery query)
			throws EvaluationException {
		return findAndSubstitute(query, 0, false);
	}

	/**
	 * Find recursive the next subgoal (the next query) by matching the given
	 * rules and facts with the current query.
	 * 
	 * @param query
	 *            The current query that should match a rule head or a fact
	 * @param recursionDepth
	 *            Current recursion depth
	 * @param inNegationAsFailureFlip
	 *            <code>true</code> if this evaluation step uses NAF,
	 ...
	 * @throws MaximumRecursionDepthReachedException
	 *             since SLDNF evaluation does not detect infinite loops, this
	 *             exception is thrown at a nesting level of 25
	 */
	private IRelation findAndSubstitute(IQuery query, int recursionDepth,
			boolean inNegationAsFailureFlip) throws EvaluationException,
		...
		if (selectedLiteral == null)
			throw new EvaluationException(
					"The selected literal must not be null.");
		...
		if (selectedLiteral.isPositive()) { // Positive Query Literal - search
			...
			List<QueryWithSubstitution> subQueryList = new LinkedList<QueryWithSubstitution>(); // List
																								// of
																								// new
																								// queries
																								// (incl.
			...
			for (QueryWithSubstitution qws : subQueryList) { // process new
																// queries
				IQuery newQuery = qws.getQuery();
				Map<IVariable, ITerm> variableMap = qws.getSubstitution();

				if (logger.isDebugEnabled()) {
					debugPrefix = getDebugPrefix(recursionDepth,
							inNegationAsFailureFlip);
				...
				if (newQuery.getLiterals().isEmpty()) {
					ITuple tuple = TopDownHelper.resolveTuple(query,
							variableMap);
				...
				IRelation relationFromSubtree = findAndSubstitute(newQuery,
						++recursionDepth, inNegationAsFailureFlip);

				logger.debug(debugPrefix + "Old query: " + query.getVariables()
						+ query);
				logger.debug(debugPrefix + "New query: "
						+ newQuery.getVariables() + newQuery + " | "
						+ variableMap);
				...
				if (relationFromSubtree.size() == 0) {
					if (!inNegationAsFailureFlip) {
						...
						logger.debug(debugPrefix + "NAF FAILURE NODE "
								+ relationReturned);
				...
				relationReturned.addAll(getFullSubgoalRelation(query, qws,
						relationFromSubtree));

				logger.debug(debugPrefix + "Return: " + relationReturned);
			}
		} else { // Negative query literal - NAF - Negation As Failure
			ILiteral queryLiteralNAF = Factory.BASIC.createLiteral(true,
					selectedLiteral.getAtom());
			IRelation relationFromNAFSubtree = findAndSubstitute(
					Factory.BASIC.createQuery(queryLiteralNAF),
					++recursionDepth, !inNegationAsFailureFlip);
			...
			if (relationFromNAFSubtree.size() == 0) {
				...
				LinkedList<ILiteral> literalsWithoutNegatedLiteral = new LinkedList<ILiteral>(
						query.getLiterals());
				literalsWithoutNegatedLiteral.remove(selectedLiteral);

				IQuery queryWithoutNegatedLiteral = Factory.BASIC
						.createQuery(literalsWithoutNegatedLiteral);

				logger.debug(debugPrefix + "Rest of query: "
						+ queryWithoutNegatedLiteral);

				// Evaluate the rest of the query
				if (!queryWithoutNegatedLiteral.getLiterals().isEmpty()) {
					relationReturned = findAndSubstitute(
							queryWithoutNegatedLiteral, ++recursionDepth,
							inNegationAsFailureFlip);
				} else {
	...
	/**
	 * Get possible sub-queries of this node by evaluating built-ins or applying
	 * rules and facts.
	 * 
	 * @param query
	 *            current query
	 ...
	 *             on failure
	 */
	private List<QueryWithSubstitution> getSubQueryList(IQuery query,
			ILiteral selectedLiteral) throws EvaluationException {
		...
		if (queryLiteralAtom instanceof IBuiltinAtom) { // BuiltIn
			subQueryList.addAll(processBuiltin(query, selectedLiteral,
					queryLiteralAtom));
		} else { // Not BuiltIn
			subQueryList
					.addAll(processQueryAgainstFacts(query, selectedLiteral));
			subQueryList
					.addAll(processQueryAgainstRules(query, selectedLiteral));
		}
		return subQueryList;
	}
	 ...
	 */
	private IRelation getFullSubgoalRelation(IQuery query,
		...
		List<IVariable> newQueryVariableList = TopDownHelper.getVariables(qws
				.getQuery());
		...
		if (relationFromSubtree.size() == 0) {
			...
			for (int i = 0; i < relationFromSubtree.size(); i++) {
				...
				Map<IVariable, ITerm> variableMapFromSubtree = TopDownHelper
						.createVariableMapFromTupleAndQuery(qws.getQuery(),
								branchTuple);
				...
				for (IVariable var : queryVariableList) {
					...
					// mappings
					ITerm termFromVariableMapping = qws.getSubstitution().get(
							var);
	 ...
	 * @param queryVariableList
	 *            list of unique variables in the query, in order of appearance
	 * @return list of queries with substitutions
	 * 
	 * @throws EvaluationException
	 *             on failure
	 */
	private List<QueryWithSubstitution> processQueryAgainstRules(IQuery query,
			ILiteral selectedLiteral) throws EvaluationException {
		...
		for (IRule rule : mRules) {
			...
			if (TopDownHelper.match(ruleHead, selectedLiteral)) {
				...
				Map<IVariable, ITerm> variableMapForOccurCheck = TopDownHelper
						.getVariableMapForVariableRenaming(rule, query);
				IRule ruleAfterOccurCheck = TopDownHelper
						.replaceVariablesInRule(rule, variableMapForOccurCheck);

				ITuple ruleHeadAfterOCTuple = ruleAfterOccurCheck.getHead()
						.get(0).getAtom().getTuple(); // ruleHead changed

				// Unifiable?
				boolean unifyable = TermMatchingAndSubstitution.unify(
						queryTuple, ruleHeadAfterOCTuple, variableMapUnify);
				...
				if (unifyable) {
					...
					// check before
					newQuery = TopDownHelper.substituteRuleHeadWithBody(query,
							selectedLiteral, ruleAfterOccurCheck);
				}
	 ...
	 * @return list of queries with substitutions
	 */
	private List<QueryWithSubstitution> processQueryAgainstFacts(IQuery query,
		...
		if (getMatchingFacts(queryLiteral, variableMapList)) {
			for (Map<IVariable, ITerm> variableMap : variableMapList) {
				...
				IQuery substitutedQuery = TopDownHelper
						.substituteVariablesInToQuery(query, variableMap);
	 ...
	 *             on failure
	 */
	private List<QueryWithSubstitution> processBuiltin(IQuery query,
	...
	/**
	 * Tries to find a fact that matches the given query. The variableMap will
	 * be populated if a matching fact was found.
	 * 
	 * @param queryLiteral
	 *            the given query
	 * @param variableMap
	 *            a <i>Map</i> that stores the resulting substitution if a fact
	 *            was found
	 * 
	 * @return true if a matching fact is found, false otherwise
	 */
	private boolean getMatchingFacts(ILiteral queryLiteral,
	 ...
	 */
	private String getDebugPrefix(int recursionDepth,
