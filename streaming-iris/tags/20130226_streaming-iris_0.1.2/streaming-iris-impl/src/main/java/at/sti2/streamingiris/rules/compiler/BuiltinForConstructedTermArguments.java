	...
	public IRelation process(IRelation leftRelation) {
		...
		for (int i = 0; i < leftRelation.size(); ++i) {
			...
			ITuple builtinInputTuple = TermMatchingAndSubstitution
					.substituteVariablesInToTuple(mBuiltinAtom.getTuple(),
							inputTuple,
			...
			if (mType == TYPE.INEQUALITY) {
				if (mPositive) {
					if (!t1.equals(t2)
							&& !mEquivalentTerms.areEquivalent(t1, t2))
				...
				boolean unified = TermMatchingAndSubstitution.unify(t1, t2,
						variableMap, mEquivalentTerms);

				if (mPositive) {
					if (unified) {
						List<ITerm> rightTerms = new ArrayList<ITerm>(
						...
						ITuple rightTuple = Factory.BASIC
								.createTuple(rightTerms);
						List<ITerm> concatenatedTerms = new ArrayList<ITerm>(
								inputTuple.size() + rightTuple.size());

						concatenatedTerms.addAll(inputTuple);
						concatenatedTerms.addAll(rightTuple);

						ITuple concatenated = Factory.BASIC
								.createTuple(concatenatedTerms);
