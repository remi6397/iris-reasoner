...
public class DemoW {
	...
	public static class MainFrame extends JFrame implements ActionListener {
		...
		private void setup() {
			...
			mProgram.setText("triple(0,0,0,1)."
					+ NEW_LINE
					...
					+ NEW_LINE + "?-exceptions_to_rule( ?a1b1,?a2b2,?n )."
					+ NEW_LINE);
			...
			mProgram.setText("republican(?x) :- like_guns(?x), not hippy(?x)."
					+ NEW_LINE
					+ "democrat(?x) :- like_hippies(?x), not republican(?x)."
					+ NEW_LINE
					+ "hippy(?x) :- like_flowers(?x), not republican(?x), not democrat(?x)."
					+ NEW_LINE + NEW_LINE + "like_flowers('a')." + NEW_LINE
					+ "like_hippies('b')." + NEW_LINE + "like_guns('c')."
					+ NEW_LINE + NEW_LINE + "?- republican(?x)." + NEW_LINE
					+ "?- democrat(?x)." + NEW_LINE + "?- hippy(?x).");

			mProgram.setText("some_calc(?X,?Z) :- MULTIPLY(?X,?X,?Y), ADD(?Y,-1, ?Z). "
					+ NEW_LINE + "?-some_calc(5,?Z).");

			mProgram.setText("p( _datetime( 1999, 12, 31, 23, 59, 59 ), _datetime( 1998, 11, 30, 22, 58, 58 ) )."
					+ NEW_LINE
