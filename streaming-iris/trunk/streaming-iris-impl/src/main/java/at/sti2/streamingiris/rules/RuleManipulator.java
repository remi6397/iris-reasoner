...
public class RuleManipulator {
	 ...
	 *            The rule to process.
	 * @param term1
	 *            The first argument to the equality.
	 * @param term2
	 *            The second argument to the equality.
	 * @return The new rule.
	 */
	public IRule addEquality(IRule rule, ITerm term1, ITerm term2) {
		...
		ILiteral literal = Factory.BASIC.createLiteral(true,
				new ExactEqualBuiltin(term1, term2));
	 ...
	 *            The rule to process.
	 * @param term1
	 *            The first argument to the inequality.
	 * @param term2
	 *            The second argument to the inequality.
	 * @return The new rule.
	 */
	public IRule addInequality(IRule rule, ITerm term1, ITerm term2) {
		...
		ILiteral literal = Factory.BASIC.createLiteral(true,
				new NotExactEqualBuiltin(term1, term2));
	 ...
	 * 
	 * @param list
	 *            the list from where to take the elements
	 ...
	 * 
	 * @param query
	 *            the query from where to take the literals
	...
	public IRule removeDuplicateLiterals(IRule rule) {
		return Factory.BASIC.createRule(rule.getHead(),
				removeDuplicates(rule.getBody()));
	}

	/**
	 * Iterate the rule looking for positive variable=variable equality
	 * built-ins. For each one found, traverse the whole rule and replace one
	...
	/**
	 * Iterate the rule looking for positive variable==constant exact
	 * equalities. For each one found, replace the variable with the constant in
	 * the other rule literals.
	 * 
	 * @param rule
	 *            The rule to process
	 * @param strict
	 *            True, do replacements only for exact equalities. False, use
	 *            any equality.
	 * @return A new rule
	 */
		...
		do {
			...
			for (ILiteral literal : rule.getBody()) {
				...
				if (!strict && !canReplace) {
					canReplace = (equality && positive)
							|| (in_equality && !positive);
				}
	...
	public IRule removeUnnecessaryEqualityBuiltins(IRule rule) {
		...
		for (ILiteral literal : rule.getBody()) {
			...
			boolean canReplace = (positive && equality)
					|| (!positive && in_equality);
	 ...
	 *            The rule to modify
	 * @param processHead
	 *            true, to modifix the head as well as the body
	 * @param remove
	 *            The term to remove
	 * @param replaceWith
	 ...
	 * @return The new rule
	 */
	public IRule replace(IRule rule, boolean processHead, ITerm remove,
			ITerm replaceWith) {
	...
	private ITuple replace(ITuple tuple, ITerm remove, ITerm replaceWith) {
		return Factory.BASIC.createTuple(replaceTerms(tuple, remove,
				replaceWith));
	}
	...
	private ITerm[] replaceTerms(ITuple tuple, ITerm remove, ITerm replaceWith) {
		...
		for (int t = 0; t < tuple.size(); ++t) {
			...
			if (oldTerm.equals(remove))
				...
				newTerms[t] = replace((IConstructedTerm) oldTerm, remove,
						replaceWith);
			else
				newTerms[t] = oldTerm;
		}
	...
	private IConstructedTerm replace(IConstructedTerm constructed,
			ITerm remove, ITerm replaceWith) {
		...
		for (ITerm oldTerm : constructed.getParameters()) {
			if (oldTerm.equals(remove))
				...
				newTerms.add(replace((IConstructedTerm) oldTerm, remove,
						replaceWith));
			else
	...
	private IAtom replace(IBuiltinAtom atom, ITerm remove, ITerm replaceWith) {
		...
		try {
			Constructor<IBuiltinAtom> constructor = (Constructor<IBuiltinAtom>) atom
					.getClass().getConstructor(newTerms.getClass());
			return constructor.newInstance((Object) newTerms);
		} catch (Exception e) {
			// should never actually get here unless soemone's been fiddling
			// about.
			throw new RuntimeException(
					"Unknown built in class type in RuleManipulator: "
