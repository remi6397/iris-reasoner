...
/**
 */
public class DayTimeDurationLessBuiltinTest extends
	...
	public void testBuiltin() throws EvaluationException {
		...
		DayTimeDurationLessBuiltin builtin = new DayTimeDurationLessBuiltin(
				date1, date1);
		...
		builtin = new DayTimeDurationLessBuiltin(date1, date2);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date1 = date2) -> null
		assertEquals(null, actual);

		builtin = new DayTimeDurationLessBuiltin(date3, date4);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date3 > date4) -> null
		assertEquals(null, actual);

		builtin = new DayTimeDurationLessBuiltin(date4, date3);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date4 < date3) -> EMPTY_TUPLE
		assertEquals(EMPTY_TUPLE, actual);

		builtin = new DayTimeDurationLessBuiltin(date4, date5);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date4 < date5) -> EMPTY_TUPLE
		assertEquals(EMPTY_TUPLE, actual);
		
		builtin = new DayTimeDurationLessBuiltin(date5, date4);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date5 > date4) -> null
		assertEquals(null, actual);
		
		builtin = new DayTimeDurationLessBuiltin(date6, date7);
		args = Factory.BASIC.createTuple(X);
		actual = builtin.evaluate(args);
		// (date6 = date7) -> null
		assertEquals(null, actual);
