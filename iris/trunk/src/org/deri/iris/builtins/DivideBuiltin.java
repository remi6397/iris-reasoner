 ...
 * </p>
 * <p>
 * $Id: DivideBuiltin.java,v 1.4 2007-05-03 11:42:59 darko_anicic Exp $
 * </p>
 * 
...
public class DivideBuiltin extends AbstractBuiltin {
	...
	public List<ITuple> evaluate(final Collection<ITuple> c) {
		...
		for(final ITuple t : c) {
			...
			if(vars[0] == 0) {
				complete[vars[0]] = BuiltinHelper.multiply(complete[1], complete[2]);
			} else if (vars[0] == 1) {
				complete[vars[0]] = BuiltinHelper.divide(complete[0], complete[2]);
			} else if (vars[0] == 2) {
				complete[vars[0]] = BuiltinHelper.divide(complete[0], complete[1]);
			} else {
