...
/**
 * A rule adaptor that decorates a rule with adornments for local
 * stratification.
 */
public class LocalStratificationDecorator {
	/**
	 * Indication of how well a term (or rule head) matches another term (or
	 * rule head).
	 */
	...
	public static class Adornment {
		 ...
		 * 
		 * @param termFromNegatedSubgoal
		 *            The 'interesting' negated sub goal from some rule
		 * @return An indication of how well this term can fulfil the
		 *         corresponding term in the negated sub-goal
		 */
		...
		/**
		 * Indicate that the rule will never produce 'constantTerm' values in
		 * the term decorated with this adornment.
		 * 
		 * @param constantTerm
		 *            The constant term.
		...
		/**
		 * Indicate that the rule will ONLY produce 'constantTerm' values in the
		 * term decorated with this adornment.
		 * 
		 * @param constantTerm
		 *            The constant term.
		 * @return The new adornment
		...
		/**
		 * The list of negated constant terms (the values will never occur
		 * here).
		 */
	...
	public LocalStratificationDecorator(IRule rule, List<Adornment> adornments) {
		assert rule.getHead().get(0).getAtom().getTuple().size() == adornments
				.size();
	...
	/**
	 * Indicate how well the given tuple (of constants and variables) matches
	 * the adornments for this rule.
	 * 
	 * @param tuple
	...
	public MatchType match(ITuple tuple) {
		...
		return partialmatch ? MatchType.CONSUMES_SUBSET
				: MatchType.CONSUMES_ALL;
	}
