...
public class LeftToRightSipTest extends TestCase {
	...
	public void testForEdges0() throws Exception {
		...
		edges.add(createEdge(createLiteral("sg", "X", "Y"),
				createLiteral("up", "X", "Z1"), X));
		edges.add(createEdge(createLiteral("up", "X", "Z1"),
				createLiteral("sg", "Z1", "Z2"), Z1));
		edges.add(createEdge(createLiteral("sg", "Z1", "Z2"),
				createLiteral("flat", "Z2", "Z3"), Z2));
		edges.add(createEdge(createLiteral("flat", "Z2", "Z3"),
				createLiteral("sg", "Z3", "Z4"), Z3));
		edges.add(createEdge(createLiteral("sg", "Z3", "Z4"),
				createLiteral("down", "Z4", "Y"), Z4));
	...
	public void testForEdges1() throws Exception {
		...
		final Set<LabeledEdge<ILiteral, Set<IVariable>>> edges = new HashSet<LabeledEdge<ILiteral, Set<IVariable>>>();
		edges.add(createEdge(createLiteral("rsg", "X", "Y"),
				createLiteral("up", "X", "X1"), X));
		edges.add(createEdge(createLiteral("up", "X", "X1"),
				createLiteral("rsg", "Y1", "X1"), X1));
		edges.add(createEdge(createLiteral("rsg", "Y1", "X1"),
				createLiteral("down", "Y1", "Y"), Y1));
	...
	public void testEqualBuiltinSip() throws Exception {
		...
		edges.add(createEdge(
				createLiteral("rsg", "Y1", "X1"),
				BASIC.createLiteral(
						true,
						BUILTIN.createEqual(TERM.createVariable("Y1"),
								TERM.createVariable("Y"))), Y1));
	...
	public void testEqualLiteralsDependency() throws Exception {
		...
		assertEquals(ax + " depends on itself.", Collections.singleton(ax),
				sip.getDepends(ax));
	}
	...
	public void testBounds0() throws Exception {
		...
		final Set<IVariable> bound_up = new HashSet<IVariable>(
				Arrays.asList(new IVariable[] { X }));
		final Set<IVariable> bound_sg = new HashSet<IVariable>(
				Arrays.asList(new IVariable[] { Z1 }));
		final Set<IVariable> bound_down = new HashSet<IVariable>(
				Arrays.asList(new IVariable[] { X, Z1, Z2 }));
		final Set<IVariable> bound_again = new HashSet<IVariable>(
				Arrays.asList(new IVariable[] { X, Z1, Z3 }));
		...
		assertEquals("Bounds of down wrong", bound_down,
				sip.getBoundVariables(down));
		assertEquals("Bounds of again wrong", bound_again,
				sip.getBoundVariables(again));
	}
	...
	public void testBounds1() throws Exception {
		...
		final Set<IVariable> bound_up = new HashSet<IVariable>(
				Arrays.asList(new IVariable[] { X }));
		final Set<IVariable> bound_rsg0 = new HashSet<IVariable>(
				Arrays.asList(new IVariable[] { X1 }));
		final Set<IVariable> bound_down = new HashSet<IVariable>(
				Arrays.asList(new IVariable[] { Y1 }));
		...
		assertEquals("Bounds of up wrong", bound_up, sip.getBoundVariables(up));
		assertEquals("Bounds of rsg0 wrong", bound_rsg0,
				sip.getBoundVariables(rsg0));
		assertEquals("Bounds of down wrong", bound_down,
				sip.getBoundVariables(down));
	}
	...
	public void testMultiHeadsMultiQueries() throws Exception {
		...
		assertEquals("The edge set does not match.", referenceEdges,
				sip.getEdges());
	}
	...
	public void testEvaluableBuiltinPassing() throws Exception {
		...
		final ILiteral less = BASIC.createLiteral(true,
				BUILTIN.createLess(X, Y));
