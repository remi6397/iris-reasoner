...
import at.sti2.streamingiris.basics.Tuple;
...
import at.sti2.streamingiris.builtins.NotExactEqualBuiltin;
import at.sti2.streamingiris.factory.Factory;
import at.sti2.streamingiris.rules.IRuleStratifier;
import at.sti2.streamingiris.rules.RuleManipulator;
import at.sti2.streamingiris.rules.stratification.LocalStratificationDecorator.Adornment;
import at.sti2.streamingiris.rules.stratification.LocalStratificationDecorator.MatchType;
import at.sti2.streamingiris.utils.StandardFloatingPointComparator;

/**
 * A local stratification algorithm. This algorithm will 'split' rules and so
 * will likely return more rules than were provided.
 * 
 * @see at.sti2.streamingiris.rules.IRuleStratifier#stratify()
 */
public class LocalStratifier implements IRuleStratifier {
	 ...
	 * 
	 * @param strict
	 *            true, if the stratifier is permitted to substitute variables
	 *            with constants when they appear in equality built-ins. false,
	 *            if the substitution may only occur when the built-in is an
	 *            exact equality
	 */
	...
	/**
	 * Indicates if the stratifier is applying strict variable-constant
	 * substitutions only.
	 * 
	...
	public List<List<IRule>> stratify(List<IRule> rules) {
		...
		while ((highest <= ruleCount) && change) {
			...
			for (int r = 0; r < mRules.size(); ++r) {
				...
				currentRule = rm.replaceVariablesWithConstants(currentRule,
						mStrict);
				...
				if (currentRule.getBody().size() == 0)
					...
					for (final ILiteral bl : currentRule.getBody()) {
						for (int r2 = 0; r2 < mRules.size(); ++r2) {
							...
							LocalStratificationDecorator adaptor = mRules
									.get(r2);

							if (adaptor.getRule().getHead().get(0).getAtom()
									.getPredicate()
									.equals(bl.getAtom().getPredicate())) {
								MatchType match = adaptor.match(bl.getAtom()
										.getTuple());
		...
		if (highest < ruleCount) {
			...
			for (int r = 0; r < mRules.size(); ++r) {
				...
				if (!GlobalStratifier.checkRuleHeadEquality(rule,
						ruleStratum[r])) {
					return null;
				}
	...
	/**
	 * For every literal of every rule, if the literal is negative and not a
	 * built-in: split any dependent rules
	 */
	...
	private boolean splitRulesForAtom(IAtom atom) {
		...
		do {
			...
			for (int ruleIndex = 0; ruleIndex < mRules.size(); ++ruleIndex) {
				...
				if (atom.getPredicate().equals(
						rule.getHead().get(0).getAtom().getPredicate())) {
					...
					if (decorator.match(negatedSubGoalTuple) == MatchType.CONSUMES_SUBSET) {
						...
						LocalStratificationDecorator exactMatchRule = makeExactMatchRule(
								decorator, negatedSubGoalTuple);
						LocalStratificationDecorator noMatchRule = makeNoMatchRule(
								decorator, negatedSubGoalTuple);
	...
	/**
	 * Create a rule that will always produce tuples that are an exact match the
	 * given rule.
	 * 
	 * @param decorator
	 *            The decorated rule.
	 * @param negatedSubGoalTuple
	 *            The tuple from the dependent rule's negated sub-goal.
	 * @return The new rule.
	 */
	private LocalStratificationDecorator makeExactMatchRule(
			LocalStratificationDecorator decorator, ITuple negatedSubGoalTuple) {
		IRule rule = decorator.getRule();
		List<LocalStratificationDecorator.Adornment> adornments = decorator
				.getAdornments();
		...
		for (int t = 0; t < adornments.size(); ++t) {
			...
			LocalStratificationDecorator.Adornment adornment = adornments
					.get(t);

			assert !headTerm.isGround() || !subGoalTerm.isGround()
					|| headTerm.equals(subGoalTerm);
			assert (adornment.getPositiveConstant() != null && headTerm
					.isGround())
					|| (adornment.getPositiveConstant() == null && !headTerm
							.isGround());
	...
	/**
	 * Create a rule that will never produce tuples that are an exact match the
	 * given rule.
	 * 
	 * @param decorator
	 *            The decorated rule.
	 ...
	 * @return The new rule.
	 */
	private LocalStratificationDecorator makeNoMatchRule(
			LocalStratificationDecorator decorator, ITuple negatedSubGoalTuple) {
		IRule rule = decorator.getRule();
		List<LocalStratificationDecorator.Adornment> adornments = decorator
				.getAdornments();
	...
	private LocalStratificationDecorator adornRule(final IRule rule) {
		...
		for (ITerm term : tuple) {
			...
			if (term.isGround())
				...
				// Term is a variable
				adornment = extractNotEqualAdornmentsFromRuleBody(rule,
						headVariable);
			}
	...
	/**
	 * Helper to scan a rule body looking for built-in predicates that imply
	 * that the variable is not equal to some constant, e.g. ?X < 'a' implies
	 * that the rules can not produce any values for ?X equal to 'a'.
	 * 
	 * @param rule
	 *            The rule to examine.
	 * @param headVariable
	 *            The variable in the head for which we are interested.
	 * @return An adornment containing all the negated constants (if any).
	 */
	private Adornment extractNotEqualAdornmentsFromRuleBody(IRule rule,
			IVariable headVariable) {
		Adornment adornment = new Adornment();
	...
	/**
	 * If we know that a variable can not be equal to a given constant term,
	 * then we can add the adornment. However, if the constant term one of the
	 * numeric types, then we can add an adornment for every numeric type.
	 * 
	 * @param adornment
	 * @param constant
	 * @return
	...
	private Adornment addNegatedConstant(Adornment adornment, ITerm constant) {
		if (constant instanceof IIntegerTerm || constant instanceof IFloatTerm
			...
			if (constant instanceof IIntegerTerm)
				adornment = adornment.addNegatedConstant(Factory.CONCRETE
						.createInteger(value.intValue()));
			else {
			...
			adornment = adornment.addNegatedConstant(Factory.CONCRETE
					.createFloat(value.floatValue()));
			adornment = adornment.addNegatedConstant(Factory.CONCRETE
					.createDouble(value.doubleValue()));
			adornment = adornment.addNegatedConstant(Factory.CONCRETE
					.createDecimal(value.doubleValue()));
		} else {
