...
import at.sti2.streamingiris.builtins.ExactEqualBuiltin;
...
public class TopDownHelper {
	...
	/**
	 * Replaces a rule head in a Query with the rule body. Example: Query: ?-
	 * a(?X), b(?X), c(?X). Rule: b(?X) :- r(?X), s(?X).
	 * 
	 * Return: ?- a(?X), r(?X), s(?X), c(?X).
	 * 
	 * Note that the rule variables and the query variables MUST NOT intersect.
	 * If they do an <code>EvaluationException</code> is thrown. To avoid
	 * intersection, do an occur check first.
	 * 
	 * @param query
	 *            a query element that contains the rule head
	 * @param selectedLiteral
	 *            the selected literal
	 * @param rule
	 *            a rule element
	 * 
	 * @return A new query element where the rule head is replaced with the rule
	 *         body, if unifyable with the selected literal.
	 * 
	 * @throws EvaluationException
	 */
	public static IQuery substituteRuleHeadWithBody(IQuery query,
			ILiteral selectedLiteral, IRule rule) throws EvaluationException {
	...
	/**
	 * Replaces variables in a query. Example: Query: ?- a( ?X ), b( ?Y ), c( ?Z
	 * ). variableMap: { X => 3, Y => 9 }
	 * 
	 * Return: ?- a( 3 ), b( 9 ), c( ?X ).
	 * 
	...
	public static IQuery substituteVariablesInToQuery(IQuery query,
		...
		for (ILiteral queryLiteral : literals) {
			ILiteral substitutedLiteral = TopDownHelper
					.substituteVariablesInToLiteral(queryLiteral, variableMap);
	...
	public static ILiteral substituteVariablesInToLiteral(ILiteral literal,
		...
		for (IVariable variable : tuple.getVariables()) {
			if (variableMap.containsKey(variable)) {
				substitutedTuple = TermMatchingAndSubstitution
						.substituteVariablesInToTuple(tuple, variableMap);
			}
		}
		for (ITerm term : tuple) {
			...
			if (term instanceof IConstructedTerm) {
				...
				LinkedList<IVariable> variables = new LinkedList<IVariable>(
						constructedTerm.getVariables());
				...
				for (IVariable variable : variables) {
					if (variableMap.containsKey(variable)) {
						substitutedTuple = TermMatchingAndSubstitution
								.substituteVariablesInToTuple(tuple,
										variableMap);
					}
				}
		...
		if (literal.getAtom() instanceof IBuiltinAtom) {
			...
			for (int i = 0; i < substitutedTuple.size(); i++) {
				...
				builtinAtom = (IBuiltinAtom) rm.replace(builtinAtom, remove,
						replaceWith);
			}
			substitutedLiteral = Factory.BASIC.createLiteral(
					literal.isPositive(), builtinAtom);
		} else {
			// No builtin
			substitutedLiteral = Factory.BASIC.createLiteral(
					literal.isPositive(), literal.getAtom().getPredicate(),
					substitutedTuple);
		}
	 ...
	 *            a predicate element
	 * @param pred2
	 *            a second predicate element
	 * 
	 * @return <code>true</code> if the predicates have the same name and arity,
	 ...
	 *            a literal element
	 * @param lit2
	 *            a second literal element
	 * 
	 * @return <code>true</code> if the predicates of the literals have the same
	 *         name and arity, <code>false</code> otherwise.
	 */
	public static boolean match(ILiteral lit1, ILiteral lit2) {
		return match(lit1.getAtom().getPredicate(), lit2.getAtom()
				.getPredicate());
	}
	...
	public static List<IVariable> getVariables(IQuery query) {
		...
		List<IVariable> variableList = query.getVariables(); // This list can
																// contain
																// variables
																// twice
		...
		for (IVariable var : variableList) {
			Integer position = variableList.indexOf(var); // indexOf takes the
															// first occurrence
	 ...
	 *            the ITuple tuple
	 * 
	 * @return A unique list of variables. The list is empty if the tuple
	...
	public static List<IVariable> getVariables(ITuple tuple) {
		...
		for (int i = 0; i < tuple.size(); i++) {
			...
			if (term instanceof IVariable
					&& !uniqueVarList.contains((IVariable) term)) {
	...
	/**
	 * Creates a variable map that will replace variable ?X with ?X1 if the
	 * variable occurs in both tuples.
	 * 
	 * @param rule
	 *            a rule
	 * @param query
	 ...
	 *             thrown if the rule contains unbound variables
	 */
	public static Map<IVariable, ITerm> getVariableMapForVariableRenaming(
			IRule rule, IQuery query) throws RuleUnsafeException {
		...
		for (IVariable var : queryVariables) {
			...
			while (ruleVariables.contains(varRename)) {
				varRename = Factory.TERM.createVariable(var.getValue()
						.toString() + ++i);
			}
	...
	/**
	 * Get all distinct variables in a rule head. No particular order is
	 * guaranteed.
	 * 
	 * @param rule
	 *            the rule
	...
	/**
	 * Get all distinct variables in a rule body. No particular order is
	 * guaranteed.
	 * 
	 * @param rule
	 *            the rule
	...
	private static Set<IVariable> getBodyVariables(IRule rule) {
		...
		for (ILiteral ruleBodyLiteral : rule.getBody()) {
			ruleVariables.addAll(ruleBodyLiteral.getAtom().getTuple()
					.getVariables());
		}
	...
	/**
	 * Given a query and a tuple, create a variable map with the corresponding
	 * variable mappings for each variable in the query.
	 * 
	 * @param query
	 *            the query
	 * @param branchTuple
	 *            the tuple
	 * 
	 * @return a new variable map containing mappings for all unique variables
	 *         in <code>query</code>
	 */
	public static Map<IVariable, ITerm> createVariableMapFromTupleAndQuery(
			IQuery query, ITuple branchTuple) {
	...
	/**
	 * Given a query and a variable map, create a tuple with the corresponding
	 * terms for each variable in the query.
	 * 
	 * @param query
	 *            the query
	 * @param variableMap
	 *            a map containing the variable mappings
	 * 
	 * @return a new tuple
	 */
	@Deprecated
	...
	/**
	 * If <code>variableMap</code> contains variable mappings of variables that
	 * are contained in <code>query</code>, those mappings will be resolved and
	 * a proper <code>tuple</code> will be created and returned.
	 * 
	 * e.g. query = ?- q(?X) variableMap = ?X = 1 return = (1)
	 * 
	 * @param query
	 *            a <code>IQuery</code> which can contain variables
	 * @param variableMap
	 *            map that stores variable mappings
	 * 
	...
	public static ITuple resolveTuple(ILiteral literal,
			Map<IVariable, ITerm> variableMap) {
		return resolveTuple(getVariables(literal.getAtom().getTuple()),
				variableMap, false);
	}

	public static ITuple resolveTuple(List<IVariable> varList,
			Map<IVariable, ITerm> variableMap, boolean partialResult) {
	...
	public static Map<IVariable, ITerm> mergeSubstitutions(
			Map<IVariable, ITerm> deeperMap, Map<IVariable, ITerm> higherMap) {
		...
		for (Entry<IVariable, ITerm> e : higherMap.entrySet()) {
			...
			substitutedTerm = TermMatchingAndSubstitution
					.substituteVariablesInToTerm(term, deeperMap);
	...
	/**
	 * Replaces all variables in a rule with the mapped ones. Always modifies
	 * the head too.
	 * 
	 * @param rule
	 *            a rule
	...
	public static IRule replaceVariablesInRule(IRule rule,
		...
		for (Entry<IVariable, ITerm> entryPair : variableMap.entrySet()) {
			replacedRule = rm.replace(replacedRule, true, entryPair.getKey(),
					entryPair.getValue());
		}
		return replacedRule;
	}
	...
	/**
	 * Does variable re-mapping. Inverse function to
	 * <code>replaceVariablesInRule(IRule, Map<IVariable, ITerm>)</code>
	 * 
	 * @param rule
	 *            a rule
	 * @param variableMap
	...
	public static IRule reMapVariablesInRule(IRule rule,
		...
		for (Entry<IVariable, ITerm> entryPair : variableMap.entrySet()) {
			if (entryPair.getValue() instanceof IVariable)
				replacedRule = rm.replace(replacedRule, true,
						entryPair.getValue(), entryPair.getKey());
		}
		return replacedRule;
	}
	 ...
	 *            depth of recursion (0 = root)
	 * @param inNegationAsFailureFlip
	 *            <code>true</code> is this a NAF tree, <code>false</code>
	 *            otherwise
	 * 
	...
	public static void printDebug(String msg, int recursionDepth,
			boolean inNegationAsFailureFlip) {
		String debugPrefix = getDebugPrefix(recursionDepth,
				inNegationAsFailureFlip);
		logger.debug(debugPrefix + msg);
	}

	/**
	 * Creates a relation by combining the relation from subgoal evaluation and
	 * the variable bindings of the current query.
	 * 
	 * e.g.
	 * 
	 * ?- p( ?X, ?Y, ?Z ) | ?- q( ?Y, ?Z ) // subgoal
	 * 
	 * When a mapping ?X = 1 is already known, and the subgoal evaluation
	 * returned ( 2, 3 ) - which means that ?Y = 2 and ?Z = 3 - a relation ( 1,
	 * 2, 3 ) for the variables ( ?X, ?Y, ?Z ) will be created.
	 * 
	 * @param query
	 *            a query
	 * @param qws
	 *            a query with proper substitution, which is a subgoal of
	 *            <code>query</code>
	 * @param relationFromSubtree
	 *            relation returned by evaluation the subgoal <code>qws</code>
	 * 
	...
	public static IRelation getFullSubgoalRelation(IQuery query,
			QueryWithSubstitution qws, IRelation relationFromSubtree) {
		return getFullSubgoalRelation(query, qws.getQuery(),
				qws.getSubstitution(), relationFromSubtree);
	}

	public static IRelation getFullSubgoalRelation(IQuery query,
			IQuery subQuery, Map<IVariable, ITerm> substitution,
		...
		if (relationFromSubtree.size() == 0) {
			...
			for (int i = 0; i < relationFromSubtree.size(); i++) {
				...
				Map<IVariable, ITerm> variableMapFromSubtree = createVariableMapFromTupleAndQuery(
						subQuery, branchTuple);
				...
				for (IVariable var : queryVariableList) {
					...
					if (termFromVariableMapping == null
							&& newQueryVariableList.contains(var)) {
						...
						if (branchTuple.isEmpty()) {
							termToAdd = var;
						} else if (branchTuple.size() != newQueryVariableList
								.size()) {
							continue;
						} else {
							termToAdd = branchTuple.get(newQueryVariableList
									.indexOf(var));
						}
					...
					if (termsPerTuple.size() == queryVariableList.size()) {
						...
						tuple = TermMatchingAndSubstitution
								.substituteVariablesInToTuple(tuple,
										variableMapFromSubtree);
						relation.add(tuple);
					}
				}
			}
	...
	public static List<QueryWithSubstitution> processBuiltin(IQuery query,
			ILiteral selectedQueryLiteral, IAtom queryLiteralAtom)
			throws EvaluationException {
		...
		if (builtinAtom instanceof EqualBuiltin
			...
			unifyable = TermMatchingAndSubstitution.unify(builtinTuple.get(0),
					builtinTuple.get(1), varMapCTarg);
		...
		if (builtinEvaluation != null) {
			...
			if (builtinTuple.getVariables().isEmpty()) {
				...
				QueryWithSubstitution qws = new QueryWithSubstitution(newQuery,
				...
				for (IVariable var : variablesPreEvaluation) {
					...
					QueryWithSubstitution qws = new QueryWithSubstitution(
							newQuery, varMap);
					newQueryList.add(qws);
