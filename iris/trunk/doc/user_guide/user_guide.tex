...
IRIS is a Datalog reasoner that can evaluate safe or unsafe datalog
extended with function symbols, XML schema data types, built-in predicates
and negation as failure
(stratified, locally stratified or un-stratified using the well-founded semantics).

It is delivered in three java `jar' files.
One contains the reasoning engine, another contains the parser and the last
contains some utility programs including two demo programs.
The demo programs are useful for experimenting with Datalog and various
evaluation options.
...
\section{Plug in External Datasources}
